...
IMGCTX PNGU_SelectImageFromBuffer (const void *buffer)
{
	...
	ctx = memalign(32, sizeof (struct _IMGCTX));
	if (!ctx)
		return NULL;
...
IMGCTX PNGU_SelectImageFromDevice (const char *filename)
{
	...
	ctx->filename = memalign(32, strlen (filename) + 1);
	if (!ctx->filename)
	{
		free (ctx);
		return NULL;
...
int PNGU_EncodeFromYCbYCr (IMGCTX ctx, PNGU_u32 width, PNGU_u32 height, void *buffer, PNGU_u32 stride)
{
	...
	ctx->img_data = memalign(32, rowbytes * height);
	if (!ctx->img_data)
	{
		png_destroy_write_struct (&(ctx->png_ptr), (png_infopp)NULL);
		if (ctx->source == PNGU_SOURCE_DEVICE)
	...
	ctx->row_pointers = memalign(32, sizeof (png_bytep) * height);
	if (!ctx->row_pointers)
	{
		png_destroy_write_struct (&(ctx->png_ptr), (png_infopp)NULL);
		if (ctx->source == PNGU_SOURCE_DEVICE)
...
int pngu_info (IMGCTX ctx)
{
	...
	if (!ctx->propRead)
	{
			...
			(png_get_tRNS (ctx->png_ptr, ctx->info_ptr, &trans, (int *) &(ctx->prop.numTrans), &trans_values)))
		{
			...
			{
				ctx->prop.trans = memalign(32, sizeof (PNGUCOLOR) * ctx->prop.numTrans);
				if (ctx->prop.trans)
...
int pngu_decode (IMGCTX ctx, PNGU_u32 width, PNGU_u32 height, PNGU_u32 stripAlpha)
{
	...
	ctx->img_data = memalign(32, rowbytes * ctx->prop.imgHeight);
	if (!ctx->img_data)
	{
		pngu_free_info (ctx);
		return PNGU_LIB_ERROR;
	...
	ctx->row_pointers = memalign(32, sizeof (png_bytep) * ctx->prop.imgHeight);
	if (!ctx->row_pointers)
	{
		free (ctx->img_data);
		pngu_free_info (ctx);
