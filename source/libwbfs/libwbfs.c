...
s32 wbfs_open_hd(rw_sector_callback_t read_hdsector,
                 rw_sector_callback_t write_hdsector,
                 void *callback_data,
                    int hd_sector_size, int num_hd_sector __attribute((unused)), int reset, wbfs_t **wbfspart)
{
	...
	for(i=0;i<4;i++,ptr+=16)
	{
		...
		{
			temp_result = wbfs_open_partition(read_hdsector,write_hdsector, callback_data,hd_sector_size,0,part_lba,reset);
			if (temp_result != NULL)
			{
				*wbfspart = temp_result;
				free(tmp_buffer);			
				return 0;
			}
		}
...
wbfs_t*wbfs_open_partition(rw_sector_callback_t read_hdsector, rw_sector_callback_t write_hdsector, void *callback_data, int hd_sector_size, int num_hd_sector, u32 part_lba, int reset)
{
		...
		{
			ret = read_hdsector(callback_data,p->part_lba,1,head);
			if (ret < 0)
			{
				return NULL;
			}
...
wbfs_disc_t *wbfs_open_disc(wbfs_t* p, u8 *discid)
{
	...
	for(i=0;i<p->max_disc;i++)
	{
		...
		{
			ret = p->read_hdsector(p->callback_data, p->part_lba+1+i*disc_info_sz_lba,1,p->tmp_buffer);
			if (ret < 0)
			{
				return NULL;
			}
			if (wbfs_memcmp(discid,p->tmp_buffer,6)==0)
			{
				...
				ret = p->read_hdsector(p->callback_data, p->part_lba+1+i*disc_info_sz_lba, disc_info_sz_lba,d->header);
				if (ret < 0)
				{
					goto error;
				}
	...
	for(i=0;i<p->max_disc;i++)
	{
		if (p->head->disc_table[i])
		{
			if(count++==index)
			{
				...
				if (magic!=0x5D1C9EA3)	// Check if the game is a Wii disc
				{
				...
				if(size)
				{
					...
					ret = p->read_hdsector(p->callback_data, p->part_lba+1+i*disc_info_sz_lba,disc_info_sz_lba,header);
					if (ret < 0)
					{
						// Error
						while (true) ;
...
u32 wbfs_extract_disc(wbfs_disc_t*d, rw_sector_callback_t write_dst_wii_sector,void *callback_data,progress_callback_t spinner)
{
        ...
        for( i=0; i< p->n_wbfs_sec_per_disc; i++)
        {
                ...
                if (iwlba)
                {
                        ...
                        ret = p->read_hdsector(p->callback_data, p->part_lba + iwlba*src_wbs_nlb, src_wbs_nlb, copy_buffer);
						if (ret < 0)
						{
							return 1;
						}
